...
def _crop_s3fd(image, boxes, labels, img_dim, rgb_means):
    height, width, _ = image.shape
...
class preproc_s3fd(object):

    def __init__(self, img_dim, rgb_means, max_expand_ratio):
        self.img_dim = img_dim
        self.rgb_means = rgb_means
    ...
    def __call__(self, image, targets):
        ...
        image_t = _distort(image)
        #image_t, boxes_t = _expand(image_t, boxes, self.rgb_means, self.max_expand_ratio)
        image_t, boxes_t, labels_t = _crop_s3fd(image_t, boxes, labels, self.img_dim, self.rgb_means)
        image_t, boxes_t = _mirror(image_t, boxes_t)
        height, width, _ = image_t.shape
        image_t = preproc_for_test(image_t, self.img_dim, self.rgb_means)
        boxes_t[:, 0::2] /= width
